...
func (p *Processor) sendEvent(eventType string, channel string, text string) error {
	...
	response := &event{Id: p.sequence, Type: eventType, Channel: channel, Text: text, Parse: "full"}
...
func filterMessage(p *Processor, data map[string]interface{}, respond messageProcessor, hear messageProcessor) {
	...
	if len(matches) == 3 {
		if respond != nil {
			m := &Message{eventStream: p, responseStrategy: reply, Text: matches[2], From: userName, fromId: userId, Channel: data["channel"].(string)}
			respond(m)
		}
	} else if data["channel"].(string)[0] == 'D' {
		if respond != nil {
			// process direct messages
			m := &Message{eventStream: p, responseStrategy: send, Text: text.(string), From: userName, fromId: userId, Channel: data["channel"].(string)}
			respond(m)
		}
	} else {
		if hear != nil {
